	...
	if(helpOver){
		...
		$.each(pathArray, function(i, activeSetItem) {
			...
			$.each(activeSetItem, function(j, activeTd ) {
				...
				$(activeTd).find('.cwd-tile-letter:contains(" ")').attr('style',' -webkit-animation-delay:'+(stepCount++)+'s;font-size:25px;').addClass('wordSlide add '+(i==1 && stepChar=='🐾'?' across ':'')).html(stepChar);
